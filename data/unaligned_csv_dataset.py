...
class UnalignedCSVDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.transform_A = get_transform(self.opt, grayscale=(input_nc == 1),channel=input_nc)
        self.transform_B = get_transform(self.opt, grayscale=(output_nc == 1), channel=output_nc)
    ...
    def __getitem__(self, index):
        ...
        if self.opt.albumentations:
            ...
            transformed = alb(image=np.array(A_img), B=np.array(B_img))
            A_img = Image.fromarray(transformed["image"])
            B_img = Image.fromarray(transformed["B"])
